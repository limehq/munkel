...
| `save-clipboard-image` | `() => Promise<string \| null>` | `session-handlers.ts` | Plan 12 P3.4. Reads the OS clipboard's image via Electron's native `clipboard` module (no `navigator.clipboard.read()` permission prompt) and saves it as a temp PNG file, returning its path in the same shape as `select-images`' entries — the palette/menu compose rows push it into `imagePaths` and it flows through `send-images` unchanged (including its imageCodec size/type limits). Returns `null` when the clipboard has no image or the temp-file write failed. |
