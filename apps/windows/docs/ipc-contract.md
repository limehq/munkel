...
| `get-palette-hotkey` | `() => Promise<string>` | `main.ts` | Plan 12 P3.1. **Sender must be the menu window** — other windows get `DEFAULT_PALETTE_HOTKEY` (`"Ctrl+Shift+M"`). Returns `currentPaletteHotkey`, the accelerator actually bound with `globalShortcut` right now (initialized from `IdentityStore#paletteHotkey` at startup, kept in sync by `set-palette-hotkey` below). |
| `set-palette-hotkey` | `(accelerator: string) => Promise<{ ok: boolean; accelerator: string; error?: 'invalid-accelerator' \| 'registration-failed' }>` | `main.ts` (logic in `palette-hotkey.ts`) | Plan 12 P3.1. **Sender must be the menu window** — other senders are rejected with `{ ok: false, accelerator: currentPaletteHotkey, error: 'registration-failed' }` and a warning log; nothing is applied or persisted. Validates the accelerator (`isValidAccelerator` — at least one modifier plus one main key, Electron accelerator format) and, if valid and different from the current binding, unregisters the old accelerator and registers the new one via `rebindPaletteHotkey`. On success, persists the new value to `IdentityStore#paletteHotkey` and returns `{ ok: true, accelerator: <new> }`. On failure (invalid format, or the OS rejected the new combo — e.g. another app already owns it), **rolls back**: re-registers the old accelerator so the hotkey is never left unbound, persists nothing, and returns `{ ok: false, accelerator: <old>, error }`. The renderer's settings-popover recorder always displays the `accelerator` the response reports, not what it requested, so a failed rebind's display snaps back to the real current binding. |
