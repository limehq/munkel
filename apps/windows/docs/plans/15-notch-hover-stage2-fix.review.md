# Review: Plan 15 — Notch hover stage-2 preview without timer reset (WIN-NOTCH-007)

## Bewertung

**Benötigt Korrektur.**

Der Plan beschreibt die richtige Architektur (orthogonaler `ui`-State neben dem zeitbasierten `phase`-State) und identifiziert die wichtigsten Stellen in `useNotchLifecycle.ts`, `NotchWidget.tsx` und `global.css`. Er ist größtenteils umsetzbar und risikoarm.

Es fehlen jedoch konkrete Änderungen für zwei IPC-Handler (`onNotchReopen`, `onNotchHide`) und die Entscheidung, ob der alte `hovering`-State komplett entfällt. Außerdem braucht es Klärung bei der Render-Priorisierung, wenn `phase === 'full'` während `ui === 'preview'`, sowie einen CSS-Selektor für den Hover-Target im Preview-Zustand.

Nachstehend sind die konkreten Korrekturvorschläge sowie eine überarbeitete Fassung des Plans.

---

## Konkrete Korrekturvorschläge

1. **`hovering` komplett entfernen, nicht als derived/internal Flag behalten.**
   - Der Plan schlägt vor, `hovering` optional zu behalten. Da `ui` alle benötigten Zustände abdeckt (`collapsed | preview | open`), ist `hovering` redundant und führt zu inkonsistenten Zwischenzuständen.
   - `setHovering` wird von `NotchWidget.tsx` nicht verwendet und kann aus der Public API entfallen.

2. **IPC-Handler `onNotchReopen` und `onNotchHide` anpassen.**
   - `onNotchReopen` setzt heute `setHovering(true)` und muss auf `setUi('open')` umgestellt werden, damit ein externer Reopen-Trigger (z. B. globaler Hotkey) direkt die volle History öffnet.
   - `onNotchHide` setzt heute `setHovering(false)` und muss auf `setUi('collapsed')` umgestellt werden.
   - Diese Stellen fehlen im aktuellen Plan vollständig.

3. **`onNotchMessage` setzt `ui` auf `collapsed`, nicht auf `open`.**
   - Der Plan ist hier korrekt, aber es sollte explizit erwähnt werden, dass `closeReply()` weiterhin aufgerufen wird.
   - Optional kann ein `useEffect` hinzugefügt werden, der `ui` auf `collapsed` zurücksetzt, sobald `history.length === 0` wird.

4. **Hover-Handler `reopenFromHoverTarget` sollte nicht in `preview` wechseln, wenn `phase === 'full'`.**
   - Während `phase === 'full'` soll die volle Ansicht ohnehin angezeigt werden. Ein Wechsel zu `preview` ist technisch möglich, aber verwirrend und überflüssig.
   - Besser: `if (history.length === 0 || phase === 'full') return;`.

5. **Render-Priorisierung in `NotchWidget.tsx` korrigieren.**
   - Die im Plan vorgeschlagene Bedingung `ui === 'open' || (phase === 'full' && ui !== 'preview')` ist unnötig komplex und widerspricht der Aussage, dass während `full` immer die volle Ansicht angezeigt wird.
   - Empfohlene Bedingung: `ui === 'open' || phase === 'full'` für den Vollansicht-Pfad, dann `ui === 'preview'` für den Preview-Pfad.

6. **CSS: `pointer-events: none` für Hover-Target auch im Preview-Zustand.**
   - Der Plan erwähnt das nur beiläufig. Es braucht eine explizite Regel, die `.notch-preview .notch-hover-target` erfasst.

7. **Tests für `onNotchReopen` und `onNotchHide` ergänzen.**
   - Automatisierte Tests sollten sicherstellen, dass ein externer Reopen `ui === 'open'` setzt und ein Hide `ui === 'collapsed'` setzt.

8. **Risiko: Interaktiver, aber unsichtbarer Fensterbereich im Preview-Zustand.**
   - Wenn `notchSetInteractive(true)` im Preview-Zustand gesetzt wird, ist das gesamte 360 × 260 px Fenster klickbar, obwohl nur ~110 px + Sliver sichtbar sind. Das kann zu versehentlichen Klicks außerhalb des sichtbaren Widgets führen. Sollte als bekanntes Restrisiko dokumentiert und im manuellen QA geprüft werden.

---

## Korrigierte Fassung des Plans
...
6. Move mouse away from full view:
   - If `phase` is `full`, view stays full for the remaining time.
...
- [ ] Manual QA on Windows confirms timer continuity and click-to-open.
