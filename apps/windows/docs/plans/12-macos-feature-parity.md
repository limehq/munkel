...
- **Bug (MAJOR):** `updateName()` in `MenuWindow.tsx:83-98` serialisiert parallele `updateProfile`-Aufrufe nicht. Wenn zwei schnelle Submits out-of-order auflösen, kann `lastSavedNameRef` auf einen älteren Namen zeigen und der Idempotenz-Guard wird inkonsistent. Fix: Request-Generation/Monotonie einführen oder Submits serialisieren, sodass der Ref nur für den zuletzt abgesetzten Namen aktualisiert wird.
- **Bug (MAJOR):** Abgelehnte `updateProfile`-Aufrufe in `MenuWindow.tsx:90-97` geben kein UI-Feedback. Der User sieht den neuen Namen im Input, geht davon aus, er sei gespeichert, und bemerkt den Fehler erst beim nächsten Öffnen. Fix: kurzzeitiger Error-State/Toast/Input-Visualisierung bei Rejection.
