...
- 2026-07-02 **[task]** Logo-Dateien sind noch nicht in die App eingearbeitet — **DONE 2026-07-06 (Placeholder)**: SVG aus `main` (`favicon.svg`) als vorläufiges Stand-in-Logo nach `apps/windows/assets/logo.svg` kopiert; `icon.ico` und Tray-PNGs neu gerendert; in `pack:dir`-Build verifiziert (PR #41). **⚠️ Das endgültige korrekte Brand-Logo steht noch aus und muss `assets/logo.svg` ersetzen, sobald es zugestellt wird.**
