...
| Notch-Auto-Ausblend/Einfahr-Sequenz fehlt | bug | idea | `apps/windows/src/renderer/components/NotchWidget.tsx` + `notch-window.ts` | Nachrichten verschwinden nicht nach Ankunft; Soll: eingeblendet → halb ausblenden/einfahren → nur Ladeanzeige → nach 60 s weg. Geplant, aber noch nicht korrekt eingearbeitet. Verwandt mit PR #22 Peek/History | 2026-07-03 |
| Menüfenster schließt nicht per Klick-außerhalb | bug | idea | `apps/windows/src/main/menu-window.ts` | Großes Menüfenster bleibt dauerhaft offen; Klick woanders schließt es nicht. Soll: `blur`/Fokusverlust → hide (click-away-to-dismiss). `alwaysOnTop:true`-Fenster braucht wahrscheinlich einen blur→hide-Handler | 2026-07-03 |
