...
| **Integrate official logo assets** | Done | — | Official brand SVG from `main` (`apps/landing/public/favicon.svg`) copied to `apps/windows/assets/logo.svg`. Icon (`assets/icon.ico`) and tray images (`tray-icon*.png`) regenerated. Verified via `bun run typecheck`, `bun test`, `bun run build`, and `bun run pack:dir`. |
